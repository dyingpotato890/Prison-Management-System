...
class Tables:
    sql_commands = [
        ...
        "CREATE TABLE IF NOT EXISTS PRISONER_DETAILS (AADHAR_NUMBER VARCHAR(20) PRIMARY KEY, NAME VARCHAR(50), AGE INT CHECK (AGE >= 18 AND AGE <= 100), NUMBER_OF_CONVICTIONS INT);",
        "CREATE TABLE IF NOT EXISTS STAFF (STAFF_ID INT PRIMARY KEY, NAME VARCHAR(50), AGE INT CHECK (AGE >= 18 AND AGE <= 70), PHONE_NUMBER VARCHAR(20), ROLE VARCHAR(50));",
        ...
        "CREATE TABLE IF NOT EXISTS WORK (PRISONER_ID INT, JOBID INT, HOURS_WORKED DECIMAL(5,2) CHECK (HOURS_WORKED >= 0 AND HOURS_WORKED <= 24), PRIMARY KEY (PRISONER_ID, JOBID), FOREIGN KEY (PRISONER_ID) REFERENCES PRISONER(PRISONER_ID), FOREIGN KEY (JOBID) REFERENCES JOBS(JOBID));"
    ]
